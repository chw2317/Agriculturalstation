...
@interface JLReleaseTaskVC ()<UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate,BLDatePickerViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource>{
...
// 可接用户
- (IBAction)needstarClick:(id)sender;
...
// 作业面积
@property (strong, nonatomic) IBOutlet UITextField *operatingarea;

// 工期要求
@property (strong, nonatomic) IBOutlet UITextField *timelimit;

// 竞标最高限价
@property (strong, nonatomic) IBOutlet UITextField *limitedprice;
...
// 具体位置
@property (strong, nonatomic) IBOutlet UITextView *detailaddress;

// 可接用户
@property (strong, nonatomic) IBOutlet UIButton *needstar;
...
@implementation JLReleaseTaskVC
...
- (void)viewDidLoad {
    ...
    _needstarArray = [NSArray arrayWithObjects:@"不限制",@"1星用户",@"2星用户",@"3星用户",@"4星用户",@"5星用户",@"6星用户", nil];
...
- (void)bl_selectedDateResultWithYear:(NSString *)year
                                month:(NSString *)month
                                  day:(NSString *)day{
    [self.starttime setTitle:[NSString stringWithFormat:@"%@-%@-%@",year, month, day] forState:UIControlStateNormal];
};
...
- (void)submitDataClick{
    ...
    // 请求地址
    NSString *url = [REQUEST_URL stringByAppendingString:@"app-task-op-release.html"];
    // 请求管理者
    ...
    parameters[@"operatingarea"] = self.operatingarea.text;
    parameters[@"timelimit"] = self.timelimit.text;
    parameters[@"limitedprice"] = self.limitedprice.text;
    parameters[@"totalprice"] = self.totalprice.text;
    parameters[@"starttime"] = [NSString stringWithFormat:@"%d",[DateUtil timeSwitchTimestamp:self.starttime.titleLabel.text andFormatter:@"yyyy-MM-dd"]];
    parameters[@"detailaddress"] = self.detailaddress.text;
    parameters[@"needstar"] = _guildStr;
...
- (void)confirmButtonAction:(UIButton *)sender{
    if([_guildStr isEqualToString:@"0"]){
        _selectStr = [NSString stringWithFormat:@"%@",_needstarArray[0]];
    }
    [self.needstar setTitle:_selectStr forState:UIControlStateNormal];
    [_bgButton removeFromSuperview];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    _selectStr = [NSString stringWithFormat:@"%@",[_needstarArray objectAtIndex:row]];
}

// 返回当前行的内容，此处是将数组中的数值添加到滚动的那个显示栏上
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    return [_needstarArray objectAtIndex:row];
}
